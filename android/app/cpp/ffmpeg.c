...
#include "cmdutils.h"
/*
 * Upstream fftools includes compat/android/binder.h here and calls
 * android_binder_threadpool_init_if_required() before transcode() (guarded
 * by CONFIG_MEDIACODEC). We skip it: this build runs FFmpeg as a library
 * inside an Android app process that already owns a binder thread pool, and
 * calling ABinderProcess_setThreadPoolMaxThreadCount() again asserts on
 * Android 15+ (API 35). libavutil.a still provides the symbol; we simply
 * never call it. See ANDROID_PATCHES.md. Re-apply this skip after every
 * upstream sync.
 */
#include "ffmpeg.h"
...
#if 0
    /*
     * Skipped for the Android app build: the app process already has a
     * binder thread pool, and ABinderProcess_setThreadPoolMaxThreadCount()
     * asserts on Android 15+ (API 35) when called again. See the note near
     * the top of this file and ANDROID_PATCHES.md. Do not re-enable on sync.
     */
