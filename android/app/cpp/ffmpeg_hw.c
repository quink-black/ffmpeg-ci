...
/* Android patch: the Java Surface set via setSurface() is injected into the
 * MediaCodec hardware device so decoded frames can be rendered to the screen.
 * See ANDROID_PATCHES.md. gSurfaceObject is declared in ffmpeg_global.h. */
...
int hw_device_init_from_string(const char *arg, HWDevice **dev_out)
{
    ...
    if (!*p) {
        ...
        if (type == AV_HWDEVICE_TYPE_MEDIACODEC) {
            /* Android patch: inject the Java Surface so the MediaCodec decoder
             * can render directly to the screen. See ANDROID_PATCHES.md. */
            ...
            {
                AVHWDeviceContext *hw_dev_ctx = device_ref->data;
                AVMediaCodecDeviceContext *media_codec = hw_dev_ctx->hwctx;
                media_codec->surface = gSurfaceObject;
            }
